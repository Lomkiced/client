...
const RecordTable = ({ records, onEdit, onArchive, onRestore, onDestroy, onView, viewMode }) => {
  ...
  const getRetentionBadge = (disposalDate) => {
    ...
    return <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">{(days / 365).toFixed(1)} Years</span>;
  };
          ...
          <tr key={record.record_id} className="hover:bg-slate-50/80 transition-colors group">
            <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${record.is_restricted ? 'bg-red-50 text-red-500' : 'bg-indigo-50 text-indigo-500'}`}>
