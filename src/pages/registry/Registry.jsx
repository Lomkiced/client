...
const Registry = () => {
  ...
  // --- DEEP LINKING HANDLER (New Implementation) ---
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const viewId = params.get('view');

    if (viewId && records.length > 0) {
      const targetRecord = records.find(r => r.record_id.toString() === viewId);
      if (targetRecord) {
        handleViewFile(targetRecord);
        // Optional: Clean URL after opening
        window.history.replaceState({}, '', '/registry');
      }
    }
  }, [records]); // Re-run when records load
